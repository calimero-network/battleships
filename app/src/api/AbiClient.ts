...
export class AbiClient {
  ...
  public async placeShips(params: { match_id: string; ships: string[] }): Promise<void> {
    const response = await this.app.execute(this.context, 'place_ships', params);
    if (response.success) {
      return response.result as void;
    } else {
      throw new Error(response.error || 'Execution failed');
  ...
  public async proposeShot(params: { match_id: string; x: number; y: number }): Promise<void> {
    const response = await this.app.execute(this.context, 'propose_shot', params);
    if (response.success) {
      return response.result as void;
    } else {
      throw new Error(response.error || 'Execution failed');
  ...
  public async acknowledgeShot(params: { match_id: string }): Promise<string> {
    const response = await this.app.execute(this.context, 'acknowledge_shot', params);
    if (response.success) {
      return response.result as string;
    } else {
      throw new Error(response.error || 'Execution failed');
  ...
  public async getOwnBoard(params: { match_id: string }): Promise<OwnBoardView> {
    const response = await this.app.execute(this.context, 'get_own_board', params);
    if (response.success) {
  ...
  public async acknowledgeShotHandler(params: { id: string; x: number; y: number }): Promise<void> {
    const response = await this.app.execute(this.context, 'acknowledge_shot_handler', params);
    if (response.success) {
      return response.result as void;
    } else {
      throw new Error(response.error || 'Execution failed');
