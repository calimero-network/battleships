...
export function useGameSubscriptions({
  ...
  const handleGameEvent = useCallback((event: AllGameEvents) => {
      ...
      if (eventData.event_type) {
        ...
        for (const executionEvent of eventData.events) {
          ...
          switch (kind) {
            ...
            case 'ShotProposed':
              return { type: 'ShotProposed', id: payload.id, x: payload.x, y: payload.y } as AllGameEvents;
            case 'ShotFired':
              return { type: 'ShotFired', id: payload.id, x: payload.x, y: payload.y, result: payload.result } as AllGameEvents;
            case 'Winner':
