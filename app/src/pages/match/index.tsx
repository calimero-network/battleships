...
export default function MatchPage() {
  ...
  const [currentContext, setCurrentContext] = useState<{ applicationId: string; contextId: string; nodeUrl: string; } | null>(null);
    ...
    (async () => {
      try {
        ...
        if (contexts.length > 0) {
          ...
          setCurrentContext({ applicationId: context.applicationId, contextId: context.contextId, nodeUrl: appUrl || 'http://node1.127.0.0.1.nip.io' });
        }
        ...
        // fetch my matches
        try { const ids = await client.getMatches(); setMyMatches(ids); } catch (_) {}
  ...
  const placeShips = useCallback(async () => {
    if (!api) return;
    if (!matchId) { show({ title: 'Set match id first', variant: 'error' }); return; }
  ...
  const proposeShot = useCallback(async (shotX?: number, shotY?: number) => {
    ...
    try {
      ...
      show({ title: `Shot proposed at (${finalX},${finalY})`, variant: 'success' });
      await loadBoards();
      await loadTurnInfo();
  ...
  const acknowledgeShot = useCallback(async () => {
    if (!api) return;
    if (!matchId) { show({ title: 'Set match id first', variant: 'error' }); return; }
    if (loadingRef.current) return;
    loadingRef.current = true;
  ...
  }, [api, matchId, show, loadBoards, loadTurnInfo]);

  const floodFillShip = (grid: boolean[][], visited: boolean[][], startX: number, startY: number, size: number): [number, number][] => {
    const ship: [number, number][] = [];
    const stack: [number, number][] = [[startX, startY]];
    
    while (stack.length > 0) {
      const [x, y] = stack.pop()!;
      if (x < 0 || x >= size || y < 0 || y >= size || visited[y][x] || !grid[y][x]) continue;
  ...
  const renderGrid = useCallback((title: string, editable: boolean)=>{
    const current = editable ? grid : Array.from({ length: size }, (_, y) => Array.from({ length: size }, (_, x) => ownBoard[(y*size)+x] === 1 || ownBoard[(y*size)+x] === 2));
    return (
      <div>
        <div style={{ marginBottom: '0.5rem', color: '#9ca3af' }}>{title}</div>
        <div style={{ display:'grid', gridTemplateColumns:`repeat(${size}, 28px)`, gap:'4px' }}>
          {current.flatMap((row, y) => row.map((cell, x) => {
            const val = editable ? (cell ? 1 : 0) : (ownBoard[(y*size)+x] || 0);
            const bg = val === 2 ? '#ef4444' : val === 3 ? '#374151' : val === 1 ? '#10b981' : '#1f2937';
            return (
              <div key={`${x}-${y}`} onClick={()=> editable && toggleCell(x,y)} style={{ width:28, height:28, background:bg, borderRadius:4, cursor: editable ? 'pointer' : 'default' }} />
            );
          }))}
        </div>
      </div>
  ...
  const renderOwnBoard = useCallback(() => {
    return (
      <div>
        <div style={{ marginBottom: '0.5rem', color: '#9ca3af' }}>Your Board</div>
        <div style={{ display:'grid', gridTemplateColumns:`repeat(${size}, 28px)`, gap:'4px' }}>
        ...
        <div style={{ marginTop: '0.5rem', fontSize: '0.75rem', color: '#9ca3af' }}>
          Legend: Empty (gray) | Ship (green) | Hit (red) | Miss (dark gray) | Pending Shot (yellow)
        </div>
      </div>
    );
  }, [ownBoard, size]);
    ...
    return (
      <div>
        <div style={{ marginBottom: '0.5rem', color: '#9ca3af' }}>
          Your Shots {isMyTurn ? '(Click to select target!)' : '(Not your turn)'}
        </div>
        <div style={{ display:'grid', gridTemplateColumns:`repeat(${size}, 28px)`, gap:'4px' }}>
          {Array.from({ length: size }, (_, y) => 
            Array.from({ length: size }, (_, x) => {
              ...
              let bg = '#1f2937'; // Default empty
              if (shotValue === 4) bg = '#f59e0b'; // Pending (yellow)
              else if (shotValue === 2) bg = '#ef4444'; // Hit (red)
              ...
              return (
                <div 
                  ...
                  style={{ 
                    ...
                    border: isSelected ? '2px solid #ffffff' : '1px solid #374151',
                    display: 'flex',
                    alignItems: 'center',
        ...
        <div style={{ marginTop: '0.5rem', fontSize: '0.75rem', color: '#9ca3af' }}>
          Legend: Empty (gray) | Pending (yellow) | Hit (red) | Miss (dark gray) | Selected (blue)
        </div>
      </div>
    );
  }, [size, shotsBoard, selectedShotX, selectedShotY, isMyTurn, handleShotGridClick]);
    ...
    return (
      <>
        ...
        <NavbarMenu align="center">
          {currentContext && (
            <div style={{ display: 'flex', gap: '1.5rem', alignItems: 'center', fontSize: '0.875rem', color: '#9ca3af', flexWrap: 'wrap', justifyContent: 'center' }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
                <Text size="sm" color="muted">Node:</Text>
                <Text size="sm" style={{ fontFamily: 'monospace', color: '#e5e7eb' }}>{currentContext.nodeUrl.replace('http://','').replace('https://','')}</Text>
              ...
              {currentUser && (
                <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
                  <Text size="sm" color="muted">Public Key:</Text>
                  <Text size="sm" style={{ fontFamily: 'monospace', color: '#e5e7eb' }}>
                    {currentUser.slice(0, 8)}...{currentUser.slice(-8)}
                  </Text>
                  <CopyToClipboard text={currentUser} variant="icon" size="small" successMessage="Public Key copied!" />
                </div>
              )}
            </div>
          )}
          ...
          <NavbarMenu align="right">
            {isAuthenticated ? (
              <Menu variant="compact" size="md"><MenuGroup><MenuItem onClick={doLogout}>Logout</MenuItem></MenuGroup></Menu>
            ) : (
              <NavbarItem><CalimeroConnectButton connectionType={{ type: ConnectionType.Custom, url: 'http://node1.127.0.0.1.nip.io' }} /></NavbarItem>
            )}
          </NavbarMenu>
        </MeroNavbar>
            ...
            <GridItem>
              <main style={{ width: '100%', maxWidth: '1000px', display: 'flex', flexDirection: 'column', gap: '2rem' }}>
                <Card variant="rounded">
                  <CardHeader><CardTitle>Create New Match</CardTitle></CardHeader>
                  <CardContent>
                    <form onSubmit={(e)=>{e.preventDefault();createMatch();}} style={{ display:'grid', gridTemplateColumns:'repeat(auto-fit, minmax(220px, 1fr))', gap:'1rem' }}>
                ...
                <Card variant="rounded">
                  <CardHeader><CardTitle>My Matches</CardTitle></CardHeader>
                  <CardContent>
                    {myMatches.length === 0 ? (
                      <div style={{ color:'#9ca3af' }}>No matches yet. Create one above!</div>
                    ) : (
                      <div style={{ display:'flex', flexDirection:'column', gap:'0.5rem' }}>
                        {myMatches.map((id)=>(
                          <div key={id} style={{ display:'flex', justifyContent:'space-between', alignItems:'center', padding:'0.75rem', backgroundColor:'#1f2937', borderRadius:'0.5rem' }}>
                            <div>
                              <Text size="sm" style={{ fontFamily:'monospace' }}>{id}</Text>
                              <div style={{ fontSize: '0.75rem', color: '#9ca3af' }}>Click to open</div>
                            </div>
                            <Button variant="primary" onClick={()=>openGame(id)}>Open</Button>
                          </div>
                        ))}
                      </div>
                    )}
    ...
    <>
      <MeroNavbar variant="elevated" size="md">
        ...
        <NavbarMenu align="left">
          <Button variant="secondary" onClick={() => setView('lobby')}>← Back to Lobby</Button>
        </NavbarMenu>
        <NavbarMenu align="center">
          {currentContext && (
            <div style={{ display: 'flex', gap: '1.5rem', alignItems: 'center', fontSize: '0.875rem', color: '#9ca3af', flexWrap: 'wrap', justifyContent: 'center' }}>
        ...
        <Grid columns={1} gap={32} maxWidth="100%" justify="center" align="center" style={{ minHeight: '100vh', padding: '2rem' }}>
          <GridItem>
            <main style={{ width: '100%', maxWidth: '1200px', display: 'flex', flexDirection: 'column', gap: '2rem' }}>
              <Card variant="rounded">
                <CardHeader><CardTitle>Match: {matchId}</CardTitle></CardHeader>
                <CardContent>
              ...
              {!placed && (
                <Card variant="rounded">
                  <CardHeader><CardTitle>Place Your Fleet</CardTitle></CardHeader>
                  <CardContent>
                        ...
                        <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
                          <div style={{ display: 'flex', gap: '0.5rem', flexWrap: 'wrap', alignItems: 'center' }}>
                            {[2, 3, 4, 5].map((len, idx) => (
                              <Button
                                key={len}
                                variant={selectedShip === idx ? "primary" : "secondary"}
                                onClick={() => setSelectedShip(selectedShip === idx ? null : idx)}
                          ...
                          <div style={{ fontSize: '0.875rem', color: '#9ca3af' }}>
                            {isRemovalMode ? 'Click ships to remove them' : 
                             selectedShip === null ? 'Click cells to toggle' : 
                             `Click to place ship of length ${selectedShip + 2} (${isHorizontal ? 'horizontal' : 'vertical'})`}
                          </div>
                          {renderGrid('Click to place ships', true)}
                          <Button type="button" variant="primary" onClick={placeShips} disabled={shipCounts.some((c, i) => c !== shipTargets[i])}>
                            Place Fleet
                          </Button>
                        </div>
                      </div>
              ...
              {placed && (
                <>
                  <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '2rem' }}>
                    <Card variant="rounded">
                      <CardHeader><CardTitle>Your Board</CardTitle></CardHeader>
                      <CardContent>
                        {renderOwnBoard()}
                      </CardContent>
                    </Card>
                        ...
                        <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
                          ...
                          <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem', alignItems: 'center' }}>
                            ...
                            <div style={{ display: 'flex', gap: '1rem', flexWrap: 'wrap', justifyContent: 'center' }}>
                              <Button 
                                variant="success" 
                                disabled={!isMyTurn || selectedShotX === null || selectedShotY === null}
                                onClick={() => selectedShotX !== null && selectedShotY !== null && proposeShot(selectedShotX, selectedShotY)}
                              >
                                {isMyTurn ? 'Fire Shot!' : 'Not Your Turn'}
                              </Button>
                              <Button type="button" variant="secondary" onClick={acknowledgeShot}>
                                Acknowledge Pending
                              </Button>
                            </div>
                            
                            <div style={{ fontSize: '0.875rem', color: '#9ca3af', textAlign: 'center', maxWidth: '500px' }}>
                              Click on an empty cell above to select your target, then click "Fire Shot!" to take your turn.
                            </div>
                            ...
                            <details style={{ width: '100%', maxWidth: '400px' }}>
                              ...
                              <form onSubmit={(e)=>{e.preventDefault();proposeShot();}} style={{ display:'grid', gridTemplateColumns:'repeat(auto-fit, minmax(120px, 1fr))', gap:'1rem', marginTop: '1rem' }}>
                                ...
                                <Button type="submit" variant="success" disabled={!isMyTurn}>
                                  Fire Manual Shot
                                </Button>
                              </form>
                            </details>
